export function copy<O>(val: O, deep = true): O {
  return deep && typeof val === 'object' && val !== null ? JSON.parse(JSON.stringify(val)) : val;
}
...
export function omit<O extends object, K extends readonly (keyof O)[]>(
  ...
  for (const key of Object.keys(obj) as (keyof O)[]) {
    if (!keys.includes(key)) omited[key as keyof typeof omited] = copy(obj[key as keyof typeof omited], deep);
  }
  return omited;
}
